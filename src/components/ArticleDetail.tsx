...
export default function ArticleDetail({ article, onBack, onMarkComplete }: Props) {
  ...
  return (
    <div className={`min-h-screen bg-slate-950 text-slate-200 font-sans ${isReadingMode ? 'pb-8 pt-8' : 'pb-24'} transition-all`}>
      {/* Header */}
      {!isReadingMode && (
        <header className="sticky top-0 z-20 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-2 bg-slate-900 border border-slate-800 text-slate-400 hover:text-white rounded-full transition-colors shrink-0"
            >
      ...
      {/* Floating Exit Reading Mode */}
      {isReadingMode && (
        <div className="fixed top-4 right-4 z-50">
          <button 
            onClick={() => setIsReadingMode(false)}
            className="p-3 bg-slate-800/80 backdrop-blur-md border border-slate-700 text-slate-300 hover:text-white rounded-full shadow-lg transition-colors"
          >
      ...
      <main className={`max-w-2xl mx-auto transition-all ${isReadingMode ? 'px-4' : ''}`}>
        ...
        <div className={`p-6 md:p-8 space-y-10 ${isReadingMode ? 'text-lg' : ''}`}>
          {/* Exact Article Text */}
          <section>
            <div className={`flex items-center gap-2 mb-4 ${isReadingMode ? 'opacity-50' : ''}`}>
          ...
          <section>
            ...
            <div className={`bg-indigo-950/20 border border-indigo-900/30 rounded-2xl p-6 text-indigo-100 leading-relaxed ${isReadingMode ? 'text-xl md:text-2xl font-serif tracking-wide leading-loose bg-transparent border-none px-0' : 'text-base'}`}>
          ...
          <section>
            ...
            <div className={`bg-orange-950/20 border border-orange-900/30 rounded-2xl p-6 text-orange-100 leading-relaxed border-l-4 border-l-orange-500 ${isReadingMode ? 'text-lg md:text-xl font-serif bg-transparent px-6' : 'text-base'}`}>
          ...
          {article.relatedIssues && (
            <section>
              ...
              <div className={`bg-rose-950/20 border border-rose-900/30 rounded-2xl p-6 text-rose-100 leading-relaxed border-l-4 border-l-rose-500 ${isReadingMode ? 'text-lg md:text-xl font-serif bg-transparent px-6' : 'text-base'}`}>
                {article.relatedIssues}
              </div>
            </section>
          )}
          ...
          {article.examples && (
            <section>
              ...
              <div className={`bg-amber-950/20 border border-amber-900/30 rounded-2xl p-6 text-amber-100 leading-relaxed border-l-4 border-l-amber-500 ${isReadingMode ? 'text-lg md:text-xl font-serif bg-transparent px-6' : 'text-base'}`}>
                {article.examples}
              </div>
            </section>
          )}
           ...
           <section>
            ...
            <div className={`text-slate-300 leading-relaxed prose prose-invert ${isReadingMode ? 'text-lg md:text-xl font-serif px-0 max-w-none' : 'text-base'}`}>
              {article.aiSummary}
            </div>
          </section>
        </div>
